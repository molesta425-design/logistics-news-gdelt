...
RISK_TERMS_RU = (
    ...
    "задержка OR надбавка OR запрет OR крушение OR столкновение OR "
    "пиратство OR наводнение OR засуха"
)
...
RSS_FEEDS = [
    {
        ...
        "ceid": "US:en",
        "query": (
            "(freight OR cargo OR shipping OR port OR maritime OR vessel "
            'OR canal OR railway OR train OR trucking OR truck OR customs '
            'OR "border crossing" OR "air cargo") '
    ...
    },
    {
        ...
        "query": (
            ...
            "железная дорога OR поезд OR вагон OR грузовик OR фура OR "
...
USER_AGENT = (
    "logistics-news-rss/2.1 "
    "(+public GitHub Actions feed)"
)
...
RULES = [
    Rule(
        ...
        (
            "Угроза безопасности, вооружённый инцидент или нападение "
            "на транспортную инфраструктуру."
        ),
        (
            "Возможны остановка движения, перенаправление грузов, "
            "рост страховых надбавок, стоимости и сроков доставки."
        ),
        "Высокая",
        100,
    ),
    Rule(
        ...
        (
            "Нужно повторно проверить допустимость груза, перевозчика "
            "и расчётов; возможны отказ в перевозке и смена маршрута."
        ),
        "Высокая",
        95,
    ),
    Rule(
        ...
        (
            "Снижается пропускная способность; вероятны очереди, "
            "отмены операций и дополнительный простой транспорта."
        ),
        "Высокая",
        90,
    ),
    Rule(
        ...
        (
            "Закрытие или временное ограничение работы маршрута, "
            "перехода либо терминала."
        ),
        (
            "Грузы потребуется перенаправлять; вероятны очереди, "
            "увеличение пробега, сроков и стоимости доставки."
        ),
        "Высокая",
        88,
    ),
    Rule(
        ...
        (
            "Возможны временная недоступность участка, задержки, "
            "дополнительная перегрузка и перенаправление грузов."
        ),
        "Высокая",
        86,
    ),
    Rule(
        ...
        (
            "Возможны ограничения движения и обработки грузов, "
            "пропуск рейсов, очереди и увеличение транзитного времени."
        ),
        "Высокая",
        84,
    ),
    Rule(
        ...
        (
            "Может вырасти время оформления; необходимо проверить "
            "документы, ограничения по грузу и доступность перехода."
        ),
        "Средняя",
        72,
    ),
    Rule(
        ...
        (
            "Перегрузка инфраструктуры и накопление "
            "необработанных грузов."
        ),
        (
            "Вероятны ожидание свободного слота, простой транспорта "
            "и рост расходов на хранение и демередж."
        ),
        "Средняя",
        70,
    ),
    Rule(
        ...
        (
            "Стоимость перевозки изменится; действующие расчёты "
            "и предложения клиентам требуется пересчитать."
        ),
        "Средняя",
        68,
    ),
    Rule(
        ...
        (
            "Возможны увеличение срока доставки, дополнительный "
            "пробег, перегрузка и рост стоимости."
        ),
        "Средняя",
        60,
    ),
...
REGIONS = [
    (
        ("black sea", "черное море", "чёрное море"),
        "Черноморский регион",
    ),
    (
...
def is_recent_article(
    ...
    oldest_allowed = reference - timedelta(
        hours=NEWS_WINDOW_HOURS
    )

    newest_allowed = reference + timedelta(
        minutes=MAX_FUTURE_SKEW_MINUTES
    )
...
def get_translator():
    ...
    if source and target:
        translator = source.get_translation(
            target
        )
...
def is_duplicate(
    ...
    for other in accepted:
        other_normalized = normalize_title(
            other
        )
...
def route_for(
    ...
    return "Международные грузовые маршруты"
...
def article_to_news(
    ...
    return {
        ...
        "assessment": (
            "Алгоритмическая значимость "
            "для грузовой логистики: "
            f"{score}/100"
        ),
    }
...
def build_language_pool(
    ...
    for article in candidates[
        ...
        accepted_titles.append(
            original_title
        )

        per_domain[domain] = (
            per_domain.get(domain, 0) + 1
        )
...
def build_feed(
    ...
    return {
        ...
        "notice": (
            "Лента содержит равное количество "
            "русскоязычных и иностранных источников. "
            "Показываются только публикации с корректной "
            "датой за последние 24 часа. Новости получены "
            "из открытых RSS-поисков Google News. Перевод "
            "выполнен локальной открытой моделью. "
            "Важность, причина и последствие — "
...
def main() -> int:
    ...
    if not articles:
        ...
        write_feed(
            empty_feed(
                (
                    "За последние 24 часа свежие "
                    "публикации не получены. "
        ...
        print(
            "Saved an empty fresh feed; "
            "stale news was removed."
        )
    ...
    if not feed["news"]:
        ...
        feed = empty_feed(
            (
                ...
                "сбалансированного набора значимых "
                "русскоязычных и иностранных новостей. "
                "Старые новости не показываются."
            )
        )
    ...
    print(
        (
            f"Saved {len(feed['news'])} "
            f"news items to {OUTPUT_PATH}"
        )
    )
